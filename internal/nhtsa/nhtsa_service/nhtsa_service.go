...
func (s *NhtsaDataServiceServer) GetMakes(ctx context.Context, req *pb.GetMakesRequest) (*pb.GetMakesResponse, error) {
	...
	// 3. Filter hanya merek terkenal (50 merek)
	popularMakes := filterPopularMakes(apiMakes)

	// 4. Simpan ke cache (di latar belakang agar tidak memblokir user)
	go s.saveMakesToCache(popularMakes)

	// 5. Konversi ke format proto dan kembalikan
	var protoMakes []*pb.Make
	for _, m := range popularMakes {
...
// Helper: Filter hanya merek mobil yang terkenal
func filterPopularMakes(allMakes []nhtsa.NhtsaMake) []nhtsa.NhtsaMake {
	// Daftar merek mobil terkenal (50 merek)
	popularBrands := map[string]bool{
		"TOYOTA": true, "HONDA": true, "FORD": true, "CHEVROLET": true,
		"NISSAN": true, "BMW": true, "MERCEDES-BENZ": true, "AUDI": true,
		"VOLKSWAGEN": true, "HYUNDAI": true, "KIA": true, "MAZDA": true,
		"SUBARU": true, "LEXUS": true, "GMC": true, "JEEP": true,
		"RAM": true, "TESLA": true, "PORSCHE": true, "LAND ROVER": true,
		"VOLVO": true, "JAGUAR": true, "MITSUBISHI": true, "SUZUKI": true,
		"BUICK": true, "CADILLAC": true, "CHRYSLER": true, "DODGE": true,
		"LINCOLN": true, "ACURA": true, "INFINITI": true, "MINI": true,
		"FIAT": true, "ALFA ROMEO": true, "MASERATI": true, "BENTLEY": true,
		"ROLLS-ROYCE": true, "FERRARI": true, "LAMBORGHINI": true, "MCLAREN": true,
		"ASTON MARTIN": true, "BUGATTI": true, "LOTUS": true, "GENESIS": true,
		"POLESTAR": true, "RIVIAN": true, "LUCID": true, "DAIHATSU": true,
		"ISUZU": true, "HINO": true,
	}

	var filtered []nhtsa.NhtsaMake
	for _, make := range allMakes {
		if popularBrands[make.MakeName] {
			filtered = append(filtered, make)
		}
	}
